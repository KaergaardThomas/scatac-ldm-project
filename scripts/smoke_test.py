...
and runs 5 training epochs end-to-end, checking all outputs.
...
def run_smoke_test(epochs: int = 5, latent_dim: int = 4):
    ...
    history = train(
        ...
        out_dir      = out_dir,
        val_frac     = 0.10,
        eval_every   = 1,       # validate every epoch so we see all metrics
    )

    # ---- Assertions ----------------------------------------------------------
    assert len(history["train_bce"])   == epochs
    assert len(history["val_auc_pr"])  == epochs
    assert all(0.0 < v < 1.5 for v in history["train_bce"]),  "BCE looks wrong"
    assert all(0.0 <= v <= 1.0 for v in history["val_auc_roc"]), "AUC-ROC out of range"
    assert all(0.0 <= v <= 1.0 for v in history["val_auc_pr"]),  "AUC-PR out of range"
