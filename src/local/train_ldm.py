...
def run_train_pipeline(
    ...
    if trainer.is_global_zero:
        ...
        print("extracting peak embeddings...")
        with torch.no_grad():
            all_peaks = torch.arange(n_peaks)
            if lightning_model.device.type != "cpu":
                all_peaks = all_peaks.to(lightning_model.device)
            peak_latent = lightning_model.model.z_j(all_peaks).cpu().numpy()

        np.save(os.path.join(model_dir, "z_peaks.npy"), peak_latent)
