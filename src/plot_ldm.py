...
def plot_peakvi(peakvi_dir: str, null_dir: str = None, out_dir: str = None):
    ...
    # ---- Figure 1: Training Loss (step-level if available, else per-epoch) ----
    ...
    if step_bce:
        ax.plot(
            step_global,
            step_bce,
            label="Train Loss (per 100 steps)",
            color="#2196F3",
            lw=1.0,
            alpha=0.8,
        )
    ...
    if eval_epochs and hist.get("val_bce"):
        ...
        # Handle potential mismatch in length between eval_epochs and val_bce safely
        ax.plot(
            x_eval[: len(hist["val_bce"])],
            hist["val_bce"],
            label="Val BCE",
            color="#F44336",
            lw=1.5,
    ...
    if null_hist and null_hist.get("val_bce"):
        ...
        ax.plot(
            null_eval[: len(null_hist["val_bce"])],
            null_hist["val_bce"],
            label="Null val BCE",
            color="#9E9E9E",
            lw=1.5,
    ...
    if eval_epochs and hist.get("val_auc_roc"):
        ...
        auc_roc_len = len(hist["val_auc_roc"])
        axes[0].plot(
            eval_epochs[:auc_roc_len],
            hist["val_auc_roc"],
            color="#4CAF50",
            lw=1.5,
            marker="o",
        ...
        if null_hist and null_hist.get("val_auc_roc"):
            null_auc_roc_len = len(null_hist["val_auc_roc"])
            axes[0].plot(
                null_hist.get("eval_epochs", [])[:null_auc_roc_len],
        ...
        auc_pr_len = len(hist["val_auc_pr"])
        axes[1].plot(
            eval_epochs[:auc_pr_len],
            hist["val_auc_pr"],
            color="#FF9800",
            lw=1.5,
            marker="o",
        ...
        if null_hist and null_hist.get("val_auc_pr"):
            null_auc_pr_len = len(null_hist["val_auc_pr"])
            axes[1].plot(
                null_hist.get("eval_epochs", [])[:null_auc_pr_len],
    ...
    if eval_epochs and hist.get("val_f1"):
        ...
        if null_hist and null_hist.get("val_f1"):
            null_f1_len = len(null_hist["val_f1"])
            ax.plot(
                null_hist.get("eval_epochs", [])[:null_f1_len],
                null_hist["val_f1"],
                color="#9E9E9E",
                lw=1.5,
                linestyle=":",
    ...
    if eval_epochs and (hist.get("val_ari") or hist.get("val_nmi")):
        ...
        if hist.get("val_ari"):
            ...
            if null_hist and null_hist.get("val_ari"):
                null_ari_len = len(null_hist["val_ari"])
                axes[0].plot(
                    null_hist.get("eval_epochs", [])[:null_ari_len],
                    null_hist["val_ari"],
                    color="#9E9E9E",
                    lw=1.5,
                    linestyle=":",
        ...
        if hist.get("val_nmi"):
            ...
            if null_hist and null_hist.get("val_nmi"):
                null_nmi_len = len(null_hist["val_nmi"])
                axes[1].plot(
                    null_hist.get("eval_epochs", [])[:null_nmi_len],
                    null_hist["val_nmi"],
                    color="#9E9E9E",
                    lw=1.5,
                    linestyle=":",
