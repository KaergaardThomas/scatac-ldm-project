...
with no latent embeddings or distance term:

    η⁰_ij = ψ_i + ω_j

It serves as a nested baseline for comparing against the full LDM.
The difference in held-out log-likelihood (BCE) and AUC-PR between the
full model and this baseline quantifies the contribution of the latent
geometric structure.
...
from train import set_seed, sample_negatives, evaluate as evaluate_ldm
...
def evaluate(
    ...
    """Evaluate null model on held-out positives + sampled negatives."""
    model.eval()
        ...
        if epoch % eval_every == 0 or epoch == 1:
            ...
            if metrics["val_auc_pr"] > best_val_auc_pr:
                ...
                torch.save(model.state_dict(),
                           os.path.join(out_dir, "best_null_model.pt"))
